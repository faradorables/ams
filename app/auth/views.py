...
from . import auth
from ..models import User, db, Inbox, Ion_Obu_List, Ion_Upgrade, Ion_Notification, Ion_User, Ion_Notification_Type, Ion_Status_Upgrade, Ion_Inbox_Upgrade
...
def request_login():
    if request.method == 'POST':
        ...
        print(apidata)
        if len(apidata['phone_number']) > 0 and len(apidata['password']) > 0:
            user = User.query.filter_by(phone_number=apidata['phone_number']).first()
            if user is not None:
                print(user.verify_password(apidata['password']))
...
@auth.route('/api/v1/delete_admin', methods=['GET', 'POST'])
@csrf.exempt
def _delete_admin():
    if request.method == "POST":
        response = {}
...
@auth.route('/api/v1/delete_message', methods=['GET', 'POST'])
@csrf.exempt
def _delete_message():
    if request.method == "POST":
        response = {}
...
@auth.route('/api/v1/add_obu', methods=['GET', 'POST'])
@csrf.exempt
def _add_obu():
    if request.method == "POST":
        response = {}
        ...
        print(apidata)
        cur_user = User.query.filter_by(id=apidata['id']).first()
        check_uid = Ion_Obu_List.query.filter_by(obu_uid=apidata['uid']).first()
        if cur_user.verify_token(apidata['token']):
            if len(apidata['uid']) > 0:
                if check_uid is None:
                    Ion_Obu_List()._insert(apidata['uid'],apidata['pin'])
                    response['status'] = '00'
                    response['messages'] = 'success'
                else:
                    response['status'] = '50'
                    response['messages'] = 'OBU UID sudah terdaftar'
            else:
                response['status'] = '50'
                response['message'] = 'OBU ID Cannot be Blank'
        else:
            response['status'] = '50'
            response['messages'] = 'user tidak terverifikasi'
        return jsonify(response)
...
@auth.route('/api/v1/delete_obu', methods=['GET', 'POST'])
@csrf.exempt
def _delete_obu():
    if request.method == "POST":
        response = {}
        ...
        cur_user = User.query.filter_by(id=apidata['id_admin']).first()
        if cur_user is not None:
            Ion_Obu_List()._delete(apidata['id'])
            response['status'] = '00'
            response['messages'] = 'success'
        else:
            response['status'] = '50'
