...
class Rdb_Transaction_Type(db.Model):
    ...
    client = db.relationship('Rdb_Client', backref='role', lazy='dynamic')
...
class Rdb_Role(db.Model):
    ...
    users = db.relationship('Rdb_User', backref='role', lazy='dynamic')
...
class Rdb_User(UserMixin, db.Model):
    ...
    def _insert(self, _name, _username, _email, _phone, _client_id):
        _user = Rdb_User()
        ...
        _user.email= _email
        _user.phone= _phone
        _user.client_id = _client_id
        _user.role_id = 1
        # _client.logo_id = '1'
        db.session.add(_user)
        db.session.commit()
        # token = _client.generate_token()
        response = Rdb_User()._data(_user.id)
        return response

    def _data(self, user_id): #untuk membaca data user kebutuhan transaksi
        response = {}
        user = Rdb_User.query.filter_by(id=user_id, status=True).first()
        if user is not None:
            response['id'] = user_id
            response['name'] = user.phone_number
...
class Ion_User(UserMixin, db.Model):
    ...
    def _count(self):
        response = {}
        response['total_user'] = Ion_User.query.filter_by(role_id=1).count()
